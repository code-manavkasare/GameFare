...
export default class AddGroups extends Component {
  ...
  async initiaLoad() {
    ...
    console.log(listGroups.hits)
    await this.setState({loader:false,listGroups:listGroups.hits,groups:groups})
    return true
  }
    ...
    return (
      <TouchableOpacity style={[styleApp.inputForm,{borderBottomWidth:0.3,borderColor:colors.borderColor,paddingTop:0,paddingBottom:10}]} activeOpacity={0.7} onPress={() => this.inputRef.focus()} >
        <Row >
          ...
          <Col size={15} style={styleApp.center} activeOpacity={0.7} onPress={() => this.searchGroups('')} >
            {this.state.search!=''?<AllIcons name='times-circle' type='font' size={13} color={colors.titles} />:null}
          </Col>
        </Row>
      </TouchableOpacity>
    )
  ...
  selectGroup (group) {
    ...
    if (Object.values(groups).filter(group1 => group1.objectID == group.objectID).length == 0) {
      groups = {
        ...groups,
      ...
      <View style={[styles.rowGroup,{flex:1}]}>
        <Row style={{marginLeft:20,width:width-40}}>       
            <Col size={15} style={styleApp.center2}>
              <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}   colors={[colors.placeHolder1, colors.placeHolder2]} style={{width:'100%',height:40,borderRadius:6}} />
            </Col>
            <Col size={75} style={[styleApp.center2,{paddingLeft:15}]}>
              <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}   colors={[colors.placeHolder1, colors.placeHolder2]} style={{width:'60%',height:14,borderRadius:2}} />
              <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}   colors={[colors.placeHolder1, colors.placeHolder2]} style={{width:'10%',height:8,borderRadius:2,marginTop:3}} />
            </Col>
        </Row>
      </View>
    )
        ...
        return (
          <Row style={{marginLeft:0,width:width-40}}>       
            ...
            <Col size={10} style={styleApp.center3}>
              {
                Object.values(this.state.groups).filter(group1 => group1.objectID == group.objectID).length != 0?
  ...
  listGroups() {
      ...
      if (this.state.listGroups.length == 0) return <Text style={[styleApp.text,{marginTop:20,paddingLeft:20,}]}>You haven't created any groups yet. Please create one first.</Text>
      return (
        <View >
          {this.state.listGroups.map((group,i) => (
      ...
      <View style={{backgroundColor:'white',flex:1,borderLeftWidth:1,borderColor:colors.off }}>
        ...
        <View style={styleApp.footerBooking}>
          <View style={{marginLeft:20,width:width-40}}>
            <Button backgroundColor={'green'} onPressColor={colors.greenClick} text={this.textButton()} click={() => this.submit()} />
          </View>
        </View>
