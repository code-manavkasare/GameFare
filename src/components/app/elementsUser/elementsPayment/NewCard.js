...
class ListEvent extends Component {
  ...
  doneScan(card) {
    ...
    numberCard= numberCard.slice(0,4) + ' ' + numberCard.slice(4,8) + ' ' + numberCard.slice(8,12) + ' ' + numberCard.slice(12,numberCard.length)
  ...
  changeCardNumber (rawText,auto) {
    ...
    this.setState({lengthCVV:lengthCVV,cardNumber:newText,digitsRequired:digitsRequired,cardType:cardType})
  }
  changedate (text) {
    var newText = ''
  ...
  close () {
    if (this.props.navigation.getParam('pageFrom')=='Checkout') return this.props.navigation.navigate('Checkout')
  ...
  payments() {
    return (
      <View style={{marginTop:0}}>
        ...
        <TouchableOpacity style={styleApp.inputForm} activeOpacity={0.7} onPress={() => this.numberTextInput.focus()}>
          <Row>
            ...
            <Col size={70} style={styleApp.center2}>
              <TextInput 
                ...
                onChangeText={text => {this.changeCardNumber(text,false)}}
                keyboardType="number-pad"
                autoFocus={true}
                underlineColorAndroid='rgba(0,0,0,0)'
                onFocus={() => {this.setState({inputFocus:'number'})}}
                onBlur={() => {this.setState({inputFocus:''})}}
            ...
            <Col size={15} style={styleApp.center} activeOpacity={0.7} onPress={() => this.props.navigation.navigate('Scan',{onGoBack:(data) => this.doneScan(data)})}>
              <AllIcons color={colors.title} size={20} name='camera' type='font' />
            </Col>
          </Row>
        </TouchableOpacity>
        ...
        <Row style={{height:55,marginTop:5,marginBottom:10}}>
            <Col size={40}>
              <TouchableOpacity style={[styleApp.inputForm,styleApp.center2]} activeOpacity = {0.8} onPress={() => {this.dateTextInput.focus()}}>
                <TextInput 
                placeholder="MM/YY"
                style={styleApp.input}
                ...
                underlineColorAndroid='rgba(0,0,0,0)'
                inputAccessoryViewID={'cardNumber'}
                onFocus={() => {this.setState({inputFocus:'date'})}}
                onBlur={() => {this.setState({inputFocus:''})}}
            ...
            <Col size={10}></Col>
            <Col size={40}>
              <TouchableOpacity style={[styleApp.inputForm,styleApp.center2]} activeOpacity = {0.8} onPress={() => {this.cvvTextInput.focus()}}>
                <TextInput 
                placeholder="CVV"
                style={styleApp.input}
                underlineColorAndroid='rgba(0,0,0,0)'
                inputAccessoryViewID={'cardNumber'}
                onChangeText={text => {this.changecvv(text)}}
                keyboardType="number-pad"
                onFocus={() => {this.setState({inputFocus:'cvv'})}}
                onBlur={() => {this.setState({inputFocus:''})}}
        ...
        <TouchableOpacity style={styleApp.inputForm} activeOpacity={0.7} onPress={() => this.zipTextInput.focus()}>
          <Row>
            <Col size={15} style={styleApp.center2}>
              <AllIcons color={colors.title} size={20} name='search-location' type='font' />
            </Col>
            <Col size={85} style={styleApp.center2}>
              <TextInput 
                ...
                underlineColorAndroid='rgba(0,0,0,0)'
                inputAccessoryViewID={'cardNumber'}
                onFocus={() => {this.setState({inputFocus:'zip'})}}
                onBlur={() => {this.setState({inputFocus:''})}}
...

        {
          ...
          <Text style={[styleApp.subtitle,{marginTop:20,fontSize:14}]}>{this.state.errorMessage}</Text>
          :null
        }
      </View>
    )      
    ...
    return (
      <View style={{backgroundColor:'white',flex:1 }}>
        ...
        <InputAccessoryView nativeID={'cardNumber'}>
            <ButtonFull backgroundColor={'green'} onPressColor={colors.greenClick} enable={!(this.state.cardNumber.length == 0 || this.state.cvv.length < 3 || this.state.expDate.length < 5 || this.state.zipCode.length == 0)} text='Confirm' click={() => this.submit()} loader={this.state.loader} />
        </InputAccessoryView>
