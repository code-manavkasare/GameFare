...
class ListEvents extends React.Component {
  ...
  listEvents(events) {
    return Object.values(events).map((event,i) => (
      <CardGroup userCard={false} pageFrom={'Event'} key={i} loadData={false} homePage={true} openGroup={(objectID) => this.openGroup(objectID)} item={this.props.allGroups[event]} data={this.props.allGroups[event]}/>
    ))
  }
  ListEvent () {
    ...
    if (!this.state.loader) {
      numberFuture = ' ('+this.props.groups.length + ')'
    }
    ...
    return (
      <View style={{marginTop:10}}>
        ...
        <Animated.View style={{height:250,borderRightWidth:0,borderColor:colors.grey,transform:[{translateX:this.translateXView1}]}}>
        <ScrollViewX 
        loader={this.state.loader}
        events={this.props.groups}
        // height={260}
        ...
        content={() => this.listEvents(this.props.groups)}
