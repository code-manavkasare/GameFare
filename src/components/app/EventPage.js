...
class EventPage extends React.Component {
  ...
  async componentDidMount() {
    if (this.props.allEvents[this.props.navigation.getParam('objectID')] == undefined) this.loadEvent(this.props.navigation.getParam('objectID'))
  }
  ...
  rowIcon (component,icon,alert) {
    ...
    color='white'
    style={[{paddingTop:10,paddingBottom:10,flex:1,borderRadius:3,marginBottom:5}]}
  ...
  rowImage (uri,text) {
    return (
        <Row style={{paddingTop:5,paddingBottom:5,marginBottom:10}}>
          <Col size={15} style={styleApp.center}>
            <AsyncImage style={{height:35,width:35,borderRadius:20,}} mainImage={uri} imgInitial={uri} />
          </Col>
          <Col size={85} style={[styleApp.center2,{paddingLeft:10}]}>
  ...
  colIcon (text,icon,click) {
    ...
    click={() => click?this.props.navigation.navigate('Alert',{close:true,title:click.title,subtitle:click.subtitle,textButton:'Close'}):null}
    color='white'
  ...
  rowUser(user,i,data) {
    return (
      <TouchableOpacity key={i} style={[{paddingTop:10,paddingBottom:10,flex:1,marginTop:10,minHeight:50,borderBottomWidth:0,backgroundColor:'white',borderColor:colors.off}]} activeOpacity={1} onPress={() => this.openProfile(user)}>
        <Row>
          <Col size={15} style={styleApp.center}>
            {
              user.info.picture !=undefined?
              <AsyncImage style={{width:35,height:35,borderRadius:17.5}} mainImage={user.info.picture} imgInitial={user.info.picture} />
              :
              <View style={[styleApp.center,{height:30,width:30,borderRadius:15,backgroundColor:colors.off2,borderWidth:1,borderColor:colors.off}]}>
                <Text style={[styleApp.input,{fontSize:11}]}>{user.info.firstname[0] + user.info.lastname[0]}</Text>
              </View>
            }

          </Col>
          <Col size={55} style={[styleApp.center2,{paddingLeft:10}]}>
            <Text style={styleApp.text}>{user.info.firstname} {user.info.lastname}</Text>
            
          </Col>
          <Col size={10} style={styleApp.center3} activeOpacity={0.7} onPress={() => !this.allowCall(user,data)?null:this.props.navigation.navigate('AlertCall',{textButton:'Close',title:user.info.firstname + ' ' + user.info.lastname,subtitle:user.info.phoneNumber,close:true,icon:<AllIcons name='envelope' type='font' color={colors.green} size={17} />})}>
          ...
          <Col size={20} style={styleApp.center} activeOpacity={0.7} onPress={() => this.props.navigation.navigate('Alert',{textButton:'Close',close:true,onGoBack:() => this.props.navigation.navigate('Event'),title:user.info.firstname + ' ' + user.info.lastname + ' is the organizer of the event.',icon:<AllIcons name='bullhorn' color={colors.blue} type='font' size={16}/>})}>
          ...
          <Col size={20} style={styleApp.center} activeOpacity={0.7} onPress={() => this.props.navigation.navigate('Alert',{textButton:'Got it!',title:'This user has been rejected by the organizer.',subtitle:user.info.firstname + ' ' + user.info.lastname,close:true,onGoBack:() => this.props.navigation.navigate('Event'),icon:<AllIcons name='close' type='mat' color={colors.primary} size={20} />})}>
            <AllIcons name='close' type='mat' color={colors.primary} size={20} />
          </Col>
          :
          <Col size={20} style={styleApp.center} activeOpacity={0.7} onPress={() => this.props.navigation.navigate('Alert',{textButton:'Got it!',title:"This user is waiting for the organizer's aproval.",subtitle:user.info.firstname + ' ' + user.info.lastname,close:true,onGoBack:() => this.props.navigation.navigate('Event'),icon:<AllIcons name='clock' type='font' color={colors.secondary} size={20} />})}>
        ...
        <View style={styleApp.marginView} >
            ...
            <Row style={{marginTop:20}}>
              <Col size={75} style={styleApp.center2}>
                <Text style={[styleApp.text,{color:colors.primary,marginTop:0,fontFamily:'OpenSans-Bold',fontSize:18}]}>{Number(data.price.joiningFee)==0?'Free entry':'$'+data.price.joiningFee}</Text>  
              </Col>
              <Col size={25} style={styleApp.center3} activeOpacity={0.7} onPress={() => this.openAlert(this.openCondition(data)?'The subscribtions for ' + data.info.name +' are open.':'The subscribtions for ' + data.info.name +' are closed.',this.openView(data))}>
                {this.openView(data)}
              </Col>
            </Row>
          ...
          <Row style={{height:90,marginTop:0}}>
            <Col>
              {this.colIcon(Number(data.info.maxAttendance)==1?data.info.maxAttendance + ' player':data.info.maxAttendance + ' players','user-plus',false)}
            </Col>
            <Col>
              {this.colIcon(level.value=='0'?level.text:level.text.split('/')[0],'balance-scale',{title:level.title,subtitle:level.subtitle})}
            </Col>
          </Row>

          <Row style={{height:90,marginTop:10,}}>
            <Col>
              {this.colIcon(data.info.gender.charAt(0).toUpperCase() + data.info.gender.slice(1),data.info.gender == 'mixed'?'venus-mars':data.info.gender == 'female'?'venus':'mars',false)}
            </Col>
            <Col>
              {this.colIcon(rule.text,'puzzle-piece',{title:rule.title,subtitle:rule.subtitle})}
            </Col>
          </Row>
  ...
  next(event,sport,rule) {
    ...
    if (this.props.infoUser.coach == true && this.props.infoUser.coachVerified == true && event.info.player == true && rule.coachNeeded) {
      return this.props.navigation.navigate('Coach',{pageFrom:'event',data:event})
    }
  ...
  async joinWaitlist (event) {
    if (!this.props.userConnected) return this.props.navigation.navigate('SignIn',{pageFrom:'Event'})
    await firebase.database().ref('events/' + event.objectID + '/waitlist').push({
      userID:this.props.userID,
  ...
  waitlistCondition(event) {
    if (!this.openCondition(event) && event.info.organizer !== this.props.userID) {
      if (event.attendees == undefined) return true
      if (Object.values(event.attendees).filter(user => user.userID === this.props.userID).length == 0) return true
    }
    return false
  }
  userAlreadySubscribed (attendees) {
    if (attendees == undefined) return true
    if (Object.values(attendees).filter(user => user.userID == this.props.userID).length == 0) return true
    return false
  }
  coachAlreadySubscribed (coaches) {
    if (coaches == undefined) return true
    if (Object.values(coaches).filter(user => user.userID == this.props.userID).length == 0) return true
    return false
  }
  async refresh() {
  ...
  render() {
    var event = this.props.allEvents[this.props.navigation.getParam('objectID')]
    if (event == undefined || this.state.loader) return <PlaceHolder />
    ...
    return (
      ...
      <ParalaxScrollView 
        ...
        image={<TouchableOpacity activeOpacity={0.3} style={{height:280,width:'100%'}}  onPress={() => {this.props.navigation.navigate('AlertAddress',{data:event.location})}}>
        <AsyncImage style={{width:'100%',height:300,borderRadius:0}} mainImage={event.images[0]} imgInitial={event.images[0]} />
        <View style={{position:"absolute",left:width/2-15,top:280/2-5}}>
           <AllIcons name='map-marker-alt' type='font' size={28} color={colors.blue} />
         </View>
       </TouchableOpacity>}

        content={() => this.event(event,this.props.navigation.getParam('loader'),sport,league,rule)} 
        header={false}
      ...
      />
...

        {
         ...
         />
          ...
          :this.openCondition(event) && this.userAlreadySubscribed(event.attendees) && this.coachAlreadySubscribed(event.coaches)?
          <FadeInView duration={300} style={styleApp.footerBooking}>
          <Button2
          icon={'next'} 
          backgroundColor='green'
