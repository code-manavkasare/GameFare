...
class ProfilePage extends Component {
  ...
  rowImage (uri,text) {
    return (
        <Row style={{paddingTop:5,paddingBottom:5}}>
          <Col size={15} style={styleApp.center}>
            <AsyncImage style={{height:40,width:40,borderRadius:20,}} mainImage={uri} imgInitial={uri} />
          </Col>
          <Col size={85} style={[styleApp.center2,{paddingLeft:10}]}>
      ...
      <View style={{marginLeft:0,width:width,marginTop:0}}>
        ...
        <View style={[{paddingTop:5}]}>
          <View style={styleApp.marginView}>
            ...
            <Text style={[styleApp.title,{marginBottom:15}]}>{data.info.name}</Text>
            ...
            {this.rowIcon(this.title(data.info.maxAttendance + ' people'),'user-check')}
        ...
        <View style={[styleApp.viewHome,{paddingTop:15}]}>
          ...
          {this.rowText(this.coach()?'Price per player':'Entry fee',colors.title,'OpenSans-SemiBold',Number(data.price.joiningFee)==0?'Free':'$' +data.price.joiningFee)}
          {this.coach()?this.rowText('Attendance',colors.title,'OpenSans-SemiBold',data.info.maxAttendance):null}
          {this.coach()?<View style={[styleApp.divider,{marginBottom:10,marginTop:10}]} />:null}
          {this.coach()?this.rowText('Get',colors.title,'OpenSans-SemiBold','$' + Number(data.price.joiningFee)*Number(data.info.maxAttendance)):null}
          {
            !this.coach() && this.props.userConnected && Number(data.price.joiningFee)!=0?
            this.rowText('Credits',colors.green,'OpenSans-SemiBold','$' +Number(this.props.totalWallet).toFixed(2))
            :null
          }

          {
            this.coach() || Number(data.price.joiningFee)==0?
            null
            :this.props.userConnected?
            <View>
              {this.rowText('Charge amount',colors.title,'OpenSans-SemiBold','$' +Math.max(0,Number(data.price.joiningFee)-Number(this.props.totalWallet)).toFixed(2))}
              <View style={[styleApp.divider2,{marginBottom:10}]} />
            </View>
            :
            <View>
              {this.rowText('Charge amount',colors.title,'OpenSans-SemiBold','$' +Number(data.price.joiningFee))}
              <View style={[styleApp.divider2,{marginBottom:10}]} />
            </View>
          }

          {
            !this.coach() && this.props.userConnected && Math.max(0,Number(data.price.joiningFee)-Number(this.props.totalWallet)) != 0?
            this.creditCard()
            :null
          }
        ...
        {
        this.coach() || Math.max(0,Number(data.price.joiningFee)-Number(this.props.totalWallet)) != 0?
        <View style={[styleApp.viewHome,{paddingTop:15}]}>
          <View style={styleApp.marginView}>
          ...
          :Math.max(0,Number(data.price.joiningFee)-Number(this.props.totalWallet)) != 0?
  ...
  textButtonConfirm(data) {
    ...
    if (Math.max(0,Number(data.price.joiningFee)-Number(this.props.totalWallet)) == 0) return 'Confirm attendance'
  ...
  async submit(data) {
    ...
    var {response,message} = await joinEvent(data,this.props.userID,this.props.infoUser,this.props.level,{tokenCusStripe:this.props.tokenCusStripe,defaultCard:this.props.defaultCard,totalWallet:this.props.totalWallet},this.props.navigation.getParam('coach'),this.props.navigation.getParam('users'))
    if (!response) {
      await this.setState({loader:false})
      return this.props.navigation.navigate('Alert',{close:true,title:message.split('. ')[0],subtitle:message.split('. ')[1],textButton:'Got it!',onGoBack:() => this.props.navigation.navigate('Checkout')})
    ...
    await  this.props.eventsAction('setAllEvents',{[data.objectID]:{
      ...data,
      [message.pushSection]:{
    ...
    return (
      <View style={[styleApp.stylePage,{borderLeftWidth:1}]}>
        ...
        <View style={styleApp.footerBooking}>
        {
          ...
          text={this.textButtonConfirm(this.props.navigation.getParam('data'))}
